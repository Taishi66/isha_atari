...
const THEMES: Record<string, Theme> = {
    ...
    solarized: {
        ...
        colors: {
            ...
            border: {
                primary: 'rgba(38, 139, 210, 0.4)',
                secondary: 'rgba(42, 161, 152, 0.5)',
                active: 'rgba(181, 137, 0, 0.8)',
            },
            background: {
                primary: 'rgba(0, 43, 54, 0.8)',
                secondary: 'rgba(7, 54, 66, 0.6)',
                active: 'rgba(181, 137, 0, 0.1)',
            },
        },
    },
};
...
export const ThemeProvider = ({ children }: ThemeProviderProps) => {
    const [currentThemeId, setCurrentThemeId] = useState<string>('cybernetic'); // Ensure default is a dark theme

    const currentTheme = THEMES[currentThemeId] || THEMES['cybernetic']; // Fallback to cybernetic if not found
    ...
    const setTheme = useCallback((themeId: string) => {
        if (THEMES[themeId] && THEMES[themeId].status !== 'locked') {
            ...
            // Handle body background for different themes
            const body = document.body;
            if (themeId === 'solarized') {
                body.style.background = THEMES['solarized']?.colors.background.primary || '#000'; // Use consistent solid primary background with fallback
            } else {
    ...
    const value: ThemeContextType = {
        ...
        themes,
        setTheme,
        colors: currentTheme.colors as ThemeColors,
    };
