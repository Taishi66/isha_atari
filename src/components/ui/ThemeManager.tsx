...
const ThemeManager = () => {
    ...
    useEffect(() => {
        if (isDragging) {
            ...
            return () => { }; // Explicitly return an empty cleanup function
        }
    ...
    return (
        ...
        >
            ...
            <div className="relative">
                    ...
                    <div className="bg-black/80 backdrop-blur-sm border min-w-[200px]" style={{ borderColor: currentTheme.colors.border.primary, backgroundColor: currentTheme.colors.background.secondary }} >
                        ...
                        <div className="border-b p-3" style={{ borderColor: currentTheme.colors.border.primary, backgroundColor: currentTheme.colors.background.primary }} >
                            <div className="flex items-center justify-between">
                                <div className="text-xs font-mono tracking-[0.2em]" style={{ color: currentTheme.colors.text.muted }}>
                                    THEME_MATRIX
                                </div>
                                    ...
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center space-x-3">
                                            ...
                                            <span className={`text-xs font-mono transition-colors duration-200 ${currentTheme.id === theme.id
                                                ...
                                                : theme.status === 'locked'
                                                    ? 'text-gray-500'
                                                    : `text-${currentTheme.colors.text.secondary} group-hover:text-[var(--theme-accent)]` // Use theme-defined secondary text color
                                                }`}>
                                                {theme.name}
                                            </span>
                                        </div>
                                        ...
                                        <div className="flex items-center space-x-2">
                                            <div
                                                className={`w-1 h-1 rounded-full ${currentTheme.id === theme.id
                                                    ? 'animate-pulse'
                                                    : theme.status === 'ready'
                                                        ? ''
                                                        : ''
                                            ...
                                            />
                                            <span
                                                className={`text-xs font-mono ${currentTheme.id === theme.id
                                                    ? ''
                                                    : theme.status === 'ready'
                                                        ? ''
                                                        : 'text-gray-500'
                        ...
                        {/* Footer */}
                        <div className="border-t p-3" style={{ borderColor: currentTheme.colors.border.primary, backgroundColor: currentTheme.colors.background.primary }}>
                            <div className="flex items-center justify-between text-xs font-mono" style={{ color: currentTheme.colors.text.muted }}>
                ...
                >
                    {/* Corner indicators */}
                    <div className={`absolute top-1 left-1 w-1 h-1 border-t border-l transition-opacity duration-300 ${isExpanded ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'
                        }`} style={{ borderColor: 'var(--theme-border-active)' }} />
                    <div className={`absolute top-1 right-1 w-1 h-1 border-t border-r transition-opacity duration-300 ${isExpanded ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'
                        }`} style={{ borderColor: 'var(--theme-border-active)' }} />
                    <div className={`absolute bottom-1 left-1 w-1 h-1 border-b border-l transition-opacity duration-300 ${isExpanded ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'
                        }`} style={{ borderColor: 'var(--theme-border-active)' }} />
                    ...
                    <div className="flex items-center space-x-3">
                        ...
                        {isExpanded ? (
                            ...
                            <ChevronUp size={12} style={{ color: currentTheme.colors.text.muted }} />
                        )}
                    </div>
